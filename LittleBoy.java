...
public class LittleBoy extends AdvancedRobot {
    ...
    static double bearingDirection = 1, lastLatVel, lastVelocity, lastReverseTime, circleDir = 1, enemyFirePower, enemyEnergy, enemyDistance, lastVChangeTime, enemyLatVel, enemyVelocity, enemyFireTime, numBadHits;
...
while (attempts++ < 100) {
    ...
    if (BF.contains(newDestination)) break;
}
        ...
        w.waveGuessFactors = guessFactors[accelIndex][bestGF][vIndex][
            BF.contains(projectMotion(robotLocation, enemyAbsoluteBearing + w.bearingDirection * GF_ZERO, enemyDistance)) ? 0 :
            BF.contains(projectMotion(robotLocation, enemyAbsoluteBearing + .5 * w.bearingDirection * GF_ZERO, enemyDistance)) ? 1 : 2
        ][distanceIndex];
    ...
    class MicroWave extends Condition {
        ...
        public boolean test() {
            if ((LittleBoy.enemyLocation).distance(firePosition) <= (distance += bulletVelocity) + bulletVelocity) {
                try {
                    waveGuessFactors[(int)Math.round((Utils.normalRelativeAngle(absoluteBearing(firePosition, LittleBoy.enemyLocation) - enemyAbsBearing)) / bearingDirection + GF_ZERO)]++;
